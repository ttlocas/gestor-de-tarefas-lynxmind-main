...
// Utilizadores fictícios (para autenticação simulada)
const MOCK_USERS = [
  {
    id: 1,
    name: "Ana Admin",
...
function App() {
  ...
  // carregar sessão guardada + dados do servidor
  useEffect(() => {
    const stored = localStorage.getItem("lynxmind_user");
    if (stored) {
      try {
  ...
  function handleLogin(email, password) {
    ...
    if (!user) {
      alert("Credenciais inválidas. Tenta outra vez.");
      return false;
    }
    const { password: _pw, ...safeUser } = user;
    setCurrentUser(safeUser);
    localStorage.setItem("lynxmind_user", JSON.stringify(safeUser));
    return true;
  }

  function handleLogout() {
    setCurrentUser(null);
    localStorage.removeItem("lynxmind_user");
  }
  ...
  function handleAddUser(newUser) {
    ...
    alert(
      "Utilizador criado (apenas no portal). Nota: o login destes utilizadores não está ligado ao backend."
    );
  }
  ...
  // se não houver user autenticado, mostra só o login
  if (!currentUser) {
  ...
  return (
    <div className="app-container" style={{ maxWidth: 480 }}>
      <header className="app-header">
        ...
        <p>Autenticação simulada com perfis de acesso (admin, gestor, colaborador).</p>
      </header>
      ...
      <section className="card">
        ...
        <div style={{ marginTop: "1rem", fontSize: "0.85rem" }}>
          ...
          <ul style={{ listStyle: "disc", paddingLeft: "1.2rem" }}>
            <li>Admin: ana@empresa.com / admin123</li>
            <li>Gestor: mario@empresa.com / gestor123</li>
            <li>Colaborador: clara@empresa.com / colab123</li>
          </ul>
        </div>
      </section>
    </div>
          ...
          <div className="users-form-card">
            ...
            <form className="task-form" onSubmit={handleSubmit}>
              ...
              <p className="users-note">
                Nota: esta gestão é simulada no frontend. Os dados não são
                guardados em base de dados nem usados para login real.
              </p>
            </form>
          </div>
        )}
...
function TaskItem({ task, onToggleStatus, onDelete, canDelete }) {
  const statusLabel =
    ...
    <li className={`task-item ${task.status}`}>
      <div className="task-main">
        ...
        <div className="task-meta">
          ...
          {task.projectName && (
            <span className="pill">Projeto: {task.projectName}</span>
          )}
        </div>
      </div>
